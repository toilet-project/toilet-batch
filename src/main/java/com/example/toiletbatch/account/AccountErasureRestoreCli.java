...
public final class AccountErasureRestoreCli {
    ...
    public static void main(String[] args) {
        try {
            ...
            var match = Pattern.compile("jdbc:mysql://127\\.0\\.0\\.1:([0-9]{4,5})/erasure_restore_[a-f0-9]{16,32}").matcher(url);
            if (!match.matches()) throw new IllegalArgumentException();
            int port = Integer.parseInt(match.group(1));
            if (port == 3306 || port > 65535) throw new IllegalArgumentException();
