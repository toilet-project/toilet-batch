...
class RegionRepositoryTest {
    ...
    @BeforeEach void setup() {
        ...
        jdbc.execute("CREATE TABLE toilet(toilet_id BIGINT PRIMARY KEY, road_address VARCHAR(255), jibun_address VARCHAR(255), latitude DECIMAL(10,7), longitude DECIMAL(10,7), coordinate_source VARCHAR(30), geocoded_at TIMESTAMP, region_revision BIGINT NOT NULL DEFAULT 1)");
        jdbc.execute("""
        ...
        jdbc.execute("""
                ...
                evaluated_latitude DECIMAL(10,7), evaluated_longitude DECIMAL(10,7), assessment_id BIGINT,
                checked_at TIMESTAMP)
                """);
    }
    @Test void existingCoordinatesAndAddressesArePreservedAndReplayIsIdempotent() throws Exception {
        jdbc.update("INSERT INTO toilet VALUES(1,'대전 유성구 대학로','대전 유성구 궁동',36.3,127.3,'ADMIN_CONFIRMED',NULL,1)");
    ...
    @Test void fillRequiresExplicitOptionVerifiedResultAndBothCoordinatesEmpty() throws Exception {
        jdbc.update("INSERT INTO toilet VALUES(1,'대전 유성구',NULL,NULL,NULL,'LEGACY',NULL,1)");
        Source source = repository.page(0, 100).getFirst();
